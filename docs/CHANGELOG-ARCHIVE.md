# CHANGELOG 归档（v0.7.1 – v1.4.10）

> 本文档归档 v1.5.0 之前的所有变更历史，供历史查阅。  
> 当前变更日志请见 [CHANGELOG.md](./CHANGELOG.md)。
...
- **触屏手势**（`static/js/gestures.js`）：侧边栏左边缘右滑打开，左滑关闭（60px 阈值）；笔记翻页滑动超 100px 跳转前/后笔记
- **跳过导航链接**（Skip Link）：键盘 Tab 聚焦时浮现，鼠标不可见
...
- **Callout 标注块**：`callout.js` 前端解析 `> [!TYPE] Title` 语法，支持 20+ 类型，支持 `[!NOTE]-` 默认收起 / `[!NOTE]+` 默认展开；`callout.css` 深/浅色模式各自配色
- **高亮语法**：`markdown.rs` 将 `==高亮文本==` 转换为 `<mark>高亮文本</mark>`
- **图片灯箱**：`lightbox.js` 点击图片弹出全屏模态层，`←`/`→` 键切换，`Esc` 关闭；`lightbox.css` 毛玻璃遮罩样式
...
### Added
- **键盘快捷键**（`static/js/keyboard.js`）：页面内导航（`j`/`k`/`g g`/`G`/`[`/`]`/`b`）、功能快捷键（`g`/`t`/`?`/`r`）、`prefers-reduced-motion` 支持
- **键盘快捷键帮助面板**（`?` 触发，`Esc` 关闭）
- **主题预设**：暖色 / 护眼 / 高对比度三套（`static/css/themes.css`）
- **自定义强调色**：设置面板颜色选择器，写入 `--accent-color` CSS 变量，localStorage 持久化
- **代码块主题独立选择**：6 套可选（auto / Atom One Dark / Light / GitHub / Dracula / Monokai）
- **交互动画**（`static/css/animations.css`）：内容区淡入、侧边栏文件夹旋转、搜索结果错峰进入、TOC 活跃链接平滑过渡；`prefers-reduced-motion` 自动禁用
...
### Changed
- **[Q1] schema_matches 字段类型检查加强**：同时比较字段名和字段类型变体，防止 TEXT→STRING 等类型变更时复用错误 schema
- **[Q3] 阅读历史自动清理**：`add_history()` 写入后自动调用 `cleanup_old_history(200)`，防止历史记录无限增长
- **[Q5] truncate_html 截断逻辑改进**：先通过状态机去除 HTML 标签提取纯文本，再按字符数截断，确保 500 字预览显示真实可见内容
- **[Q6] 持久化分批写入**：笔记按 1000 条分批提交，元数据最后写入（原子完成标记），降低单次事务锁库时长
- **[Q7] metrics 指标注册去除 expect**：`init_metrics()` 改用 `let _ =` 静默忽略 AlreadyRegistered 错误，防止测试环境 panic
...
- **[P5] graph.rs 消除 content_text 解析**：`extract_links_from_note` 改用 `Note.outgoing_links` 预计算字段，消除图谱生成时的正则解析开销
...
### Fixed
- **[S2] Cookie 补充 `Secure` 和 `SameSite` 属性**：登录/登出 Cookie 添加 `.secure(true).same_site(SameSite::Lax)`，防止 JWT Token 在 HTTP 连接下明文传输
- **[S3] 分享链接密码改用 bcrypt 哈希存储**：`password` 字段重命名为 `password_hash`，创建时使用 `bcrypt` 单向哈希，验证时使用 `bcrypt::verify`，防止数据库泄露时密码明文暴露
- **[S4] 认证中间件路径匹配收紧**：`/login`、`/api/auth/login` 从 `starts_with` 改为精确匹配 `==`，防止 `/login-admin` 等路径绕过认证
