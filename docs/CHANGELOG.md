...
### Added
- **`GET /api/graph/path?from=&to=`**（`src/handlers.rs`、`src/graph.rs`）：服务端 BFS 路径查找，最多 6 跳，返回路径节点+边列表、跳数、无路径消息
- **`find_shortest_path()`**（`src/graph.rs`）：BFS + 前驱映射，支持从同节点查找（0 跳）、直接/多跳路径、超过 max_hops 时返回 None
- **图谱专页路径查找 UI**（`templates/graph_page.html`）：工具栏"路径查找"按钮切换面板；起点/终点输入框（支持 datalist 自动补全）；路径节点金色高亮、非路径半透明；显示跳数和节点列表（可点击跳转）

### 测试统计
- 服务端：130/130 通过（新增 5 个路径查找测试）

---

## [v1.9.1] — 2026-04-15

代码审计修复版本（CODEREVIEW_1.9）。

### Fixed
- **[Q1] WASM 模块版本注释过期**（`crates/wasm/src/lib.rs`）：`v1.8.6` → `v1.9.0`

### 审计统计
- 🔵 P3 修复：1 项（Q1）
- 接受为设计限制：2 项（I1 String 克隆、I2 f32 vs f64 精度差异）
- PageRank 算法审计：数值溢出/收敛性/除零/边界处理全部确认正确 ✅
...
### Fixed
- **[B1] export_html zip.write_all 错误被静默忽略**（`src/handlers.rs`）：`let _` 改为 `if let Err(e) = ... { error!(...) }`，确保写入失败时有日志记录而不是产生无声损坏的 zip 条目

### Changed
- **[Q1] WASM 模块版本注释**（`crates/wasm/src/lib.rs`）：`v1.8.0` → `v1.8.6`

### 审计统计
- 🟡 P2 修复：1 项（B1）
- 🔵 P3 修复：1 项（Q1）
- 发现问题总计：2 项（另 4 项：2 项确认正确，2 项接受设计限制）
