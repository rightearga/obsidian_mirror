...
**当前版本**: v1.8.7 🎉  
**下一里程碑**: v1.8.x 系列已完成 🎉，进入 v1.9.x 规划  
**长期规划**: v1.9.x 方向待定（AI 语义搜索候选）  
**最后更新**: 2026-04-15
...
> v1.8 系列主题：**规模化 · 发布 · 离线 · 可视化**
>
> 覆盖四个独立方向（E/B/C/D），每版聚焦单一主题，以代码审计收尾。

---

### ✅ v1.8.0 (已发布 - 2026-04-15) — 规模化性能优化

**主题**：面向 10,000+ 笔记库的体验优化，消除大规模场景下的卡顿瓶颈

#### 侧边栏虚拟滚动

- 当前侧边栏将所有 SidebarNode 全量渲染为 DOM，5000+ 文件时初始化耗时 > 1s 且滚动卡顿
- 改为"可视窗口渲染"（IntersectionObserver + 固定行高），仅保持约 80 个 DOM 节点在页面中
- 折叠/展开状态依然持久化到 localStorage

#### 搜索结果分页

- `/api/search` 新增 `page`（默认 1）和 `per_page`（默认 20，最大 100）参数
- 前端搜索结果底部加载更多按钮（无限滚动可选）
- 避免一次返回 500+ 条结果对 DOM 造成的渲染压力

#### 全局图谱渐进式加载

- 超过 800 节点时分批加载：先展示有连接的核心节点，孤立节点延迟加载
- 进度条提示加载状态

#### 实际交付物
- 修改文件：`src/search_engine.rs`（SearchPage + advanced_search_paginated + top_docs_to_results）
- 修改文件：`src/handlers.rs`（SearchQuery 加 page/per_page，search_handler 升级）
- 修改文件：`static/js/search.js`（分页响应处理 + 加载更多）
- 修改文件：`static/js/init.js`（requestIdleCallback 延迟初始化）
- 修改文件：`static/css/sidebar.css`（content-visibility: auto）
- 修改文件：`static/js/graph.js`（两阶段渐进式加载）
- 修改文件：`templates/graph_page.html`（同步渐进式加载逻辑）

#### 测试结果
- 服务端全量测试：**119/119 通过**
- 新增测试：4 个（分页第一页、最后一页、超出范围、per_page 截断）

---

### ✅ v1.8.1 (已发布 - 2026-04-15) — 代码审计（CODEREVIEW_1.8）

**主题**：对 v1.8.0 引入的新代码进行系统性审查

#### 修复内容
- ✅ **[B1] `page` 无上限 → usize 溢出风险**：`page.min(10_000)` 加上限，防止 Modified 排序路径整数溢出
- ✅ **[Q1] WASM 模块版本号未更新**：`v1.7.0` → `v1.8.0`

#### 设计限制（已知接受）
- ⏸ I1：`content-visibility: auto` 旧 Firefox 优雅降级
- ⏸ I2：`concat` 创建新数组，分页步长小无实际影响
...
### ✅ v1.8.2 (已发布 - 2026-04-15) — 导出与发布

**主题**：让笔记内容以多种形式对外发布

#### PDF 导出

- `GET /doc/{path}?print=1` 触发专用打印样式（隐藏侧边栏、优化分页、显示页眉页脚）
- 纯 CSS `@media print` 实现，无服务端依赖，浏览器"打印为 PDF"即可
- 笔记页面添加"导出 PDF"按钮（调用 `window.print()`）

#### RSS / Atom 订阅

| 路由 | 说明 |
|------|------|
| `GET /feed.xml` | 全库最近 50 篇笔记（按 mtime 降序） |
| `GET /feed.xml?tag={tag}` | 指定标签的笔记 |
| `GET /feed.xml?folder={path}` | 指定文件夹的笔记 |

- Atom 1.0 格式，标准 RSS 阅读器可订阅
- `<content>` 包含完整渲染 HTML

#### 静态站点导出

- `POST /api/export/html` → 将整个 vault 渲染为静态 HTML 文件树，打包为 zip 下载
- 可直接部署到 GitHub Pages / Netlify（无需服务器）
- 排除认证相关功能，仅保留笔记内容和导航

#### 实际交付物
- 新增文件：`static/css/print.css`（打印媒体查询）
- 修改文件：`templates/layout.html`（引入 print.css）
- 修改文件：`templates/page.html`（"打印"按钮）
- 修改文件：`src/handlers.rs`（feed_handler + export_html_handler + xml_escape + to_rfc3339 + 静态 HTML 生成 + hmac 0.13 适配）
- 修改文件：`src/main.rs`（路由注册）
- 修改文件：`Cargo.toml`（zip 依赖 + hmac/sha2 版本同步）

#### 测试结果
- 服务端全量测试：**123/123 通过**
- 新增测试：4 个（xml_escape、path 转换、静态 HTML 结构、rfc3339 格式）

---

### ✅ v1.8.3 (已发布 - 2026-04-15) — PWA 离线完善

**主题**：在 v1.6.x 离线搜索基础上，将核心浏览体验延伸到断网场景

#### 扩展离线缓存策略

- 已访问笔记的 HTML（Cache-First + 后台刷新）
- 侧边栏数据（`/api/titles`）离线可用
- 图谱数据（`/api/graph/global`）首次加载后缓存

#### 离线状态 UI

- 状态栏加网络状态指示器（在线/离线切换时动画提示）
- 离线时搜索框提示"使用离线索引搜索"
- 无缓存页面离线访问时显示友好提示（非白屏）

#### 后台同步提示

- 当 Service Worker 检测到 `/sync` 完成后，通过 Broadcast Channel 通知前端刷新数据
- 页面顶部显示"有新内容，点击刷新"横幅（不强制跳转）

#### 实际交付物
- 修改文件：`static/sw.js`（CACHE_NAME v3、DATA_CACHE_NAME、staleWhileRevalidate、POST /sync 通知、离线搜索响应格式升级）
- 修改文件：`static/js/pwa.js`（网络状态监听、SYNC_COMPLETE 消息监听）
- 修改文件：`templates/layout.html`（刷新横幅、网络状态指示器、离线搜索提示 div）
- 修改文件：`static/js/search.js`（X-Offline-Search 头检测）

#### 测试结果
- 服务端全量测试：**123/123 通过**（纯前端变更）

---

### ✅ v1.8.4 (已发布 - 2026-04-15) — 可视化增强

**主题**：充分利用 v1.7.x 积累的图谱、洞察和时间数据，构建更丰富的知识视图

#### 时间线视图（`GET /timeline`）

- 按 frontmatter `date` 或 mtime 展示笔记时间轴
- 支持按月/年折叠，支持标签过滤
- 点击跳转对应笔记，悬停预览内容

#### 知识热力图（图谱增强）

- 全局图谱新增"热度叠加"模式：节点颜色映射修改频率（蓝→红）
- 链接粗细映射被引用次数（粗 = 高入度中心节点）

#### 洞察 Dashboard 增强

- 新增"最活跃笔记排行"（被引用最多 Top 10）
- 笔记网络密度趋势（每月新增链接数折线图）
- 孤立笔记占比历史变化（减少 = 知识库越来越紧密）

#### 实际交付物
- 修改文件：`src/domain.rs`（GraphNode 加 `mtime: i64`）
- 修改文件：`src/graph.rs`（所有节点填充 mtime）
- 修改文件：`src/insights.rs`（LinkedNoteEntry、MonthlyLinkCount、most_linked_notes、monthly_link_counts）
- 修改文件：`src/sync.rs`（compute_insights 传入 backlinks）
- 修改文件：`src/handlers.rs`（timeline_handler、timeline_api_handler、titles mtime、extract_note_date_str）
- 修改文件：`src/main.rs`（路由注册）
- 修改文件：`src/templates.rs`（TimelineTemplate）
- 新增文件：`templates/timeline.html`
- 修改文件：`templates/graph_page.html`（热度着色）
- 修改文件：`templates/insights.html`（排行 + 趋势图）
- 修改文件：`templates/layout.html`（时间线侧边栏按钮）

#### 设计限制
- ⏸ "孤立笔记占比历史变化"需要历史快照数据，当前架构按同步时点计算，不记录历史，暂不实现

#### 测试结果
- 服务端全量测试：**125/125 通过**
- 新增测试：2 个（most_linked_notes 排序/空值）

---

### ✅ v1.8.5 (已发布 - 2026-04-15) — 依赖升级

**主题**：升级全部核心依赖至最新稳定版，清理技术债

#### 核心依赖升级结果

| 依赖 | 起始版本 | 升级后版本 | 备注 |
|------|---------|---------|------|
| actix-web | 4.12.1 | **4.13.0** | ✅ 自动升级 |
| tantivy | 0.25.0 | **0.26.0** | ✅ 自动升级 |
| redb | 3.1.0 | **4.0.0** | ✅ 自动升级（主版本跳跃）|
| pulldown-cmark | 0.13.0 | **0.13.3** | ✅ 自动升级 |
| wasm-bindgen | 0.2.118 | **0.2.118** | 保持（wasm-pack 兼容性约束）|
| tokio | 1.49.0 | **1.52.0** | ✅ 自动升级 |
| hmac | 0.12 | **0.13.0** | ✅ 自动升级，API 变更已适配（SimpleHmac）|
| sha2 | 0.10 | **0.11.0** | ✅ 自动升级 |
| bcrypt | 0.18.0 | **0.19.0** | ✅ 自动升级 |
| jieba-rs | 0.8.1 | **0.9.0** | ✅ 自动升级 |
| rayon | 1.11 | **1.12.0** | ✅ 自动升级 |
| zip | （新增）| **8.5.1** | v1.8.2 引入 |

#### 安全审计

- `cargo-audit` 工具未安装，需手动执行：`cargo install cargo-audit && cargo audit`
- 当前版本组合下全量测试 125/125 服务端 + 38/38 WASM 全部通过，无已知兼容性问题
- 注：hmac 0.13 API 变更（`Hmac<D>` → `SimpleHmac<D>` + `KeyInit` trait）已在 v1.8.2 适配

#### 实际交付物
- 修改文件：`CLAUDE.md`（关键依赖版本表更新）
- 修改文件：`.claude/project.md`（依赖版本速查表更新）
...
### ✅ v1.8.6 (已发布 - 2026-04-15) — 性能回归修复（PERFORMANCE-1.8 改进项）

**主题**：修复 v1.8.x 开发周期内引入的两项性能回退，恢复并超越基准水平

> 依据：`docs/PERFORMANCE-1.8.md` 基准报告识别的两处问题

#### P1：WASM NoteIndex 搜索性能回退（高优先级）

**问题**：`v1.8.5` 基准显示 NoteIndex 搜索 ASCII 达 1855µs，CJK 2015µs——分别比 v1.7.x（~200µs / ~620µs）慢 8× / 3×。

**根因**：v1.7.0 M4 优化将评分阶段改为"迭代 TokenCache 三个字段集合建 u8 位掩码"。当 `content_tokens` 集合较大（1000 条笔记，每条数百 tokens）时，迭代所有 content_tokens = O(n_tokens)，反而慢于原 `HashSet.contains()` 的 O(1) 查询。

**修复方案**：回退 M4，恢复原始评分策略（对每个 query_token ≤8 逐一查三个 HashSet）

```rust
// 恢复 v1.6.x 策略：query_tokens × 3 次 HashSet.contains()
// 优于 M4 的 TokenCache 迭代，因 query_tokens 数量有限（≤8）
...
**预期收益**：ASCII 1855µs → ~200µs（**-89%**），CJK 2015µs → ~620µs（**-69%**）

**文件**：`crates/wasm/src/lib.rs`（`search_json` 评分阶段）

---

#### P2：图谱 mtime 热路径优化（低优先级）

**问题**：v1.8.4 为 `GraphNode` 新增 `mtime: i64` 字段，图谱构建时每节点调用 `SystemTime::duration_since(UNIX_EPOCH)`。`graph/200` 从 ~80µs 升至 192µs（**+140%**）。

**修复方案**：在 `AppState` 中增加 `mtime_cache: HashMap<String, i64>`（路径→mtime 秒）；`perform_sync` 后填充；`generate_graph` / `generate_global_graph` 直接读取，避免热路径重复转换。

**预期收益**：graph/200 192µs → ~80µs（**-58%**）

**文件**：`src/state.rs`、`src/sync.rs`、`src/graph.rs`

---

#### 验证目标（运行 `cargo bench` 对比 v1.8.5 基准）

| 指标 | v1.8.5 实测 | v1.8.6 目标 | 改善预期 |
|------|------------|------------|---------|
| WASM NoteIndex ASCII | 1855 µs | ~200 µs | **-89%** |
| WASM NoteIndex CJK | 2015 µs | ~620 µs | **-69%** |
| graph/local_graph_depth2/200 | 192 µs | ~80 µs | **-58%** |

#### 实际交付物
- 修改文件：`crates/wasm/src/lib.rs`（回退 M4 评分方案）
- 修改文件：`src/graph.rs`（generate_global_graph 内 mtime 预计算）
- 修改文件：`src/state.rs`（AppState.mtime_cache 字段）
- 修改文件：`src/sync.rs`（填充 mtime_cache）

#### 实测收益（cargo bench）
| 指标 | v1.8.5 | v1.8.6 | 改善 |
|------|--------|--------|------|
| WASM NoteIndex ASCII | 1855 µs | **192 µs** | **-89.5%** |
| WASM NoteIndex CJK | 2015 µs | **509 µs** | **-73.5%** |
...
### ✅ v1.9.1 (已发布 - 2026-04-15) — 代码审计（CODEREVIEW_1.9）

**主题**：对 v1.9.0 引入的新代码进行系统性审查

#### 修复内容
- ✅ **[Q1] WASM 模块版本注释过期**：`v1.8.6` → `v1.9.0`

#### 技术验证结论
- ✅ PageRank 数值无溢出风险，20 轮/0.85 阻尼收敛可靠
- ✅ 边权重计算逻辑正确，借用安全（先 HashSet 后 mut 遍历）
- ✅ GraphNode.pagerank 无安全风险（计算结果，非用户输入）

#### 设计限制（已知接受）
- ⏸ I1：边权重 HashSet 查找 String 克隆，典型规模无影响
- ⏸ I2：服务端 f32 vs WASM f64 精度差异，均足够精确
...
以下方向已全部纳入 v1.7.x / v1.8.x 规划：

1. **笔记聚类分析** → v1.7.0 图谱聚类着色
2. **版本历史查看** → v1.7.2 Git 版本历史
3. **笔记洞察 Dashboard** → v1.7.3 健康度 + 写作趋势
4. **多仓库切换** → v1.7.4 多仓库支持
5. **规模化性能** → v1.8.0 虚拟滚动 + 搜索分页
6. **导出与发布** → v1.8.2 PDF + RSS + 静态站
7. **PWA 离线完善** → v1.8.3 更多页面离线 + 状态提示
8. **可视化增强** → v1.8.4 时间线 + 热力图 + 洞察增强
...
2. **评论系统** — 不符合只读展示定位；替代方案：GitHub Issues
3. **协作编辑** — 与只读定位冲突；替代方案：Obsidian 客户端 + Git
4. **插件系统** — 增加安全风险和维护成本
5. **键盘快捷键自定义** — 增加配置复杂度
