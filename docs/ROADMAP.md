...
**主题**: 代码审计修复（CODEREVIEW_1.4）— 路径遍历 P0 修复 + 认证与状态管理改进

#### 修复内容
- ✅ **[S1] assets_handler 路径遍历漏洞**：添加 `is_path_within` 函数，用 `canonicalize` + 前缀验证防止 `../../` 类路径越界
- ✅ **[B1] sync_status 失败时不更新**：在 `perform_sync` 中使用 RAII 守卫，异常退出时自动设为 FAILED
- ✅ **[Q1] API 认证失败应返回 401 JSON**：中间件对 `/api/*` 路径返回 `{"error":"未认证"}` + 401，而非 302 重定向
- ✅ **[Q4] git.rs 韩文注释**：替换为中文
- ✅ **[T1] assets_handler 路径遍历回归测试**：8 个新测试（is_path_within + assets_handler + webhook 签名验证）
