...
- [ ] **代码块主题独立选择**
  - 可用方案：GitHub Light / GitHub Dark / Dracula / Monokai / One Dark
  - 深色模式默认 One Dark，浅色模式默认 GitHub Light
  - 设置面板统一管理
...
- [ ] **页面切换过渡**
  ...
  - 可在设置中关闭（减少视觉干扰 / 性能敏感场景）
- [ ] **侧边栏动画优化**
  - 文件夹展开/折叠弹性动画（cubic-bezier）
  - 搜索结果列表项依次错峰进入（stagger 20ms）
- [ ] **TOC 高亮平滑滚动**
  - 当前章节指示器使用滑动条而非跳跃切换
...
- [ ] **实时搜索建议**
  - 笔记标题补全：输入时从 `link_index` 实时过滤，无需网络请求
  - 标签补全：输入 `#` 后触发，从 `tag_index` 过滤
  - 键盘导航（`↑`/`↓` 选择，`Enter` 确认，`Esc` 关闭）
- [ ] **搜索结果改进**
  - 高亮匹配关键词（加粗）
  - 显示笔记路径 + 标签 + 最后修改时间
  - 摘要片段基于纯文本（复用 v1.3.4 `truncate_html` 逻辑）
- [ ] **搜索历史管理**
  - 清空全部历史按钮
  - 单条删除（悬停显示 `×`）
  - 最多保留 20 条（旧的自动淘汰）
...
- [ ] **孤立笔记页面** (`GET /orphans`)
  - 列出无任何出链且无入链的笔记（利用 `backlinks` + `outgoing_links` 计算）
  - 帮助用户发现未被整合的笔记
  - 侧边栏入口（统计面板下方）
- [ ] **随机笔记** (`GET /random` → 重定向到随机笔记)
  - 侧边栏"随机漫游"按钮
  - 快捷键：`r` 触发随机跳转
- [ ] **最近更新页面** (`GET /recent`)
  - 按修改时间降序列出笔记
...
- [ ] **全局图谱视图** (`GET /graph/global`)
  - 展示整个笔记库的链接关系，不限于当前笔记邻域
  - 侧边栏"全局图谱"入口
  - 节点数 > 500 时自动降采样（仅显示引用数 ≥ 1 的笔记）
- [ ] **图谱筛选面板**
  - 按标签过滤（多选，OR 逻辑）
  - 按文件夹过滤
  - 隐藏孤立节点开关
...
**Q4 (10-12月)**: 深度功能期
- 完成 v1.4.3（图谱增强：全库图谱、节点着色、布局多样化）
