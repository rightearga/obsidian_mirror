...
pub async fn save_progress_handler(
    ...
    // 获取现有进度或创建新进度
    let mut progress = match app_state.reading_progress_db.get_progress(&username, &body.note_path) {
        Ok(Some(p)) => p,
        Ok(None) => {
            ReadingProgress::new(
                username.clone(),
    ...
    // 保存到数据库
    if let Err(e) = app_state.reading_progress_db.save_progress(&progress) {
        error!("保存阅读进度失败: {}", e);
        return HttpResponse::InternalServerError().json(serde_json::json!({
            "error": "保存阅读进度失败"
        }));
...
pub async fn get_progress_handler(
    ...
    // 获取进度
    match app_state.reading_progress_db.get_progress(&username, &note_path) {
        Ok(Some(progress)) => {
...
pub async fn list_progress_handler(
    ...
    // 获取所有进度
    match app_state.reading_progress_db.get_user_progress(&username, limit) {
        Ok(progress_list) => {
...
pub async fn delete_progress_handler(
    ...
    // 删除进度
    match app_state.reading_progress_db.delete_progress(&username, &note_path) {
        Ok(true) => {
            HttpResponse::Ok().json(serde_json::json!({
                "message": "删除成功"
            }))
...
pub async fn add_history_handler(
    ...
    // 添加到数据库
    if let Err(e) = app_state.reading_progress_db.add_history(&history) {
        error!("添加阅读历史失败: {}", e);
        return HttpResponse::InternalServerError().json(serde_json::json!({
            "error": "添加阅读历史失败"
        }));
...
pub async fn list_history_handler(
    ...
    // 获取历史记录
    match app_state.reading_progress_db.get_user_history(&username, limit) {
        Ok(history_list) => {
