...
- `insights_cache: TokioRwLock<InsightsCache>` — 笔记洞察缓存（v1.7.3），每次同步后更新；v1.8.4 新增 `most_linked_notes`（被引用 Top N，`LinkedNoteEntry`）和 `monthly_link_counts`（每月新增链接数，`MonthlyLinkCount`）
