...
> 📋 **[CODEREVIEW 报告](docs/CODEREVIEW_1.9.md)**：v1.9.x 系列代码审计报告
...
- ✅ **反向链接**：显示所有指向当前笔记的链接
- ✅ **关系图谱**：全库图谱（`/graph/global`）+ 局部图谱；节点大小按入度/PageRank、颜色按标签分组；力导向/层级布局；图谱内搜索；PNG 导出；路径查找（BFS 最短路径）
- ✅ **笔记目录**：自动生成 TOC，滚动高亮当前章节
- ✅ **面包屑导航**：显示路径层级
- ✅ **全局知识图谱专页**：`/graph` 全屏独立页，Sigma.js 2 WebGL 渲染（10k+ 节点 60fps），聚类着色，工具栏，孤立节点开关，影响力模式（PageRank），路径查找（v1.7.0, v1.9.x）
- ✅ **Git 版本历史**：`/doc/{path}/history` 提交历史列表，`/at/{commit}` 快照，`/diff/{commit}` 行级 diff（v1.7.2）
- ✅ **笔记洞察 Dashboard**：`/insights` 写作趋势、知识库健康度（断链/孤立/超大）、标签云；标签共现矩阵；笔记连通度评分；阅读频率热力图（v1.7.3, v1.9.3）
- ✅ **多仓库切换**：导航栏仓库下拉切换器，`/r/{name}/...` 路由前缀（v1.7.4）
- ✅ **搜索结果分页**：`/api/search` 支持 page/per_page，"加载更多"按钮（v1.8.0）
- ✅ **时间线视图**：`/timeline` 按 frontmatter date/mtime 时间轴，月/年折叠，标签过滤；写作速度折线图；标签演化堆叠面积图；月份→图谱联动（v1.8.4, v1.9.4）
- ✅ **知识地图**：`/knowledge-map` 全屏 Canvas 星图，Jaccard 标签相似度聚类，K-means 分组，WASM ForceAtlas2 布局，PageRank 节点大小模式（v1.9.5）
...
- ✅ **WASM 加速**：客户端渲染（render_markdown）、离线搜索（NoteIndex）、ForceAtlas2 图谱布局（v1.6.x, v1.9.7）；Bitset 优化 CJK 搜索速度（M3/M4/M5）；知识地图 Jaccard+K-means+F-R 布局（v1.9.5）
...
### 🧪 质量保证
- ✅ **单元测试**：133 个服务端测试 + 46 个 WASM 测试 = 179 个测试（100% 通过），含 search_engine / handlers 集成测试
...
- **搜索引擎**: Tantivy（倒排索引）
- **中文分词**: jieba-rs
- **图谱可视化**: Sigma.js 2（WebGL）+ Canvas（知识地图）
- **认证**: JWT + bcrypt
- **持久化**: postcard + redb
- **监控**: Prometheus
...
| GET | `/knowledge-map` | 知识地图全屏专页（标签相似度聚类，v1.9.5） |
| GET | `/api/knowledge-map` | 知识地图数据 JSON（笔记 + tags + pagerank，v1.9.5） |
| GET | `/api/graph/path` | 笔记最短路径（BFS，`?from=&to=`，v1.9.2） |
...
- **v1.9.x**：图谱引擎升级（Sigma.js 2 WebGL，10k+ 节点 60fps）、WASM ForceAtlas2 布局（2k 节点 <1s）、知识地图（Jaccard 聚类星图，Canvas 渲染）、PageRank 影响力量化、图谱路径查找（BFS）、洞察深化（标签共现矩阵/连通度/阅读热力图）、时间线联动（写作速度/标签演化/图谱跳转）、时间线性能优化（content-visibility/IntersectionObserver）、移动端手势修复
...
**项目地址**: https://github.com/rightearga/obsidian_mirror  
**当前版本**: v1.9.8 🎉  
